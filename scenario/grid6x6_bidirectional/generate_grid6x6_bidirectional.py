...
NET_PATH = SCENARIO_DIR / "grid6x6_bidirectional.net.xml"
ROUTES_PATH = SCENARIO_DIR / "grid6x6_bidirectional_routes.rou.xml"
SUMOCFG_PATH = SCENARIO_DIR / "grid6x6_bidirectional.sumocfg"
VIEW_SETTINGS_PATH = SCENARIO_DIR / "grid6x6_bidirectional_view.settings.xml"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build a reusable 6x6 bidirectional SUMO traffic grid scenario.")
    parser.add_argument("--seed", type=int, default=42, help="Deterministic SUMO seed written to grid6x6_bidirectional.sumocfg")
    ...
    parser.add_argument("--grid-cols", type=int, default=6, help="Number of traffic-light columns")
    parser.add_argument("--grid-rows", type=int, default=6, help="Number of traffic-light rows")
    parser.add_argument(
        "--base-vph",
        type=int,
        default=240,
...
def build_emergency_routes(net, routes_root: ET.Element) -> None:
    xs, ys, node_by_coord, _node_by_id = _grid_lookup(net)
    ...
    ev_specs = [
        (
            "ev_route_0",
            "ev_0",
            _route_from_waypoints(net, [n(0, 0), n(5, 0), n(5, 2), n(3, 2), n(3, 5)]),
            "20",
        ),
        (
            "ev_route_1",
            "ev_1",
            _route_from_waypoints(net, [n(0, 5), n(5, 5), n(5, 1), n(0, 1), n(0, 4)]),
            "70",
        ),
        (
            "ev_route_2",
            "ev_2",
            _route_from_waypoints(net, [n(0, 0), n(5, 0), n(5, 2), n(0, 2), n(0, 4), n(5, 4), n(5, 5), n(2, 5)]),
            "120",
        ),
    ]
    ...
    for route_id, vehicle_id, edge_ids, depart in ev_specs:
        ...
        if route_id == "ev_route_0" and not (10 <= len(edge_ids) <= 12):
            raise RuntimeError(f"{route_id} must be 10-12 edges, got {len(edge_ids)}")
        if route_id == "ev_route_1" and not (16 <= len(edge_ids) <= 18):
            raise RuntimeError(f"{route_id} must be 16-18 edges, got {len(edge_ids)}")
        if route_id == "ev_route_2" and not (22 <= len(edge_ids) <= 25):
            raise RuntimeError(f"{route_id} must be 22-25 edges, got {len(edge_ids)}")
...
def write_routes_file(args: argparse.Namespace) -> None:
    ...
    build_emergency_routes(net, routes_root)
...
<configuration xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    ...
    <input>
        <net-file value="grid6x6_bidirectional.net.xml"/>
        <route-files value="grid6x6_bidirectional_routes.rou.xml"/>
    </input>
    ...
    <gui_only>
        <gui-settings-file value="grid6x6_bidirectional_view.settings.xml"/>
    </gui_only>
</configuration>
"""
